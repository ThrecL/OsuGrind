...
public class LazerScoreDetector
{
        ...
        try
        {
            ...
            if (isPlaying && !_isReplaySession && _lastState == 2 && LastSnapshot != null)
            {
                ...
                if (currentTime < _lastTime - 500 && _lastTime > 1000)
                {
                    ...
                    _hasRecordedCurrentPlay = false; _wasPlaying = true; _liveTimeline.Clear(); _ppTimeline.Clear();
            ...
            if (snapshot.StateNumber != _lastState)
            {
                ...
                {
                    _hasRecordedCurrentPlay = false; _wasPlaying = true; _isReplaySession = snapshot.IsReplay;
                    _liveTimeline.Clear(); _ppTimeline.Clear();
                    _lastCombo = 0; _lastMisses = 0; _lastH300 = 0; _lastH100 = 0; _lastH50 = 0;
                    _lastSliderTail = 0; _lastSmallTick = 0; _lastLargeTick = 0;
            ...
            {
                int combo = snapshot.Combo ?? 0, misses = snapshot.HitCounts?.Misses ?? 0;
                int h300 = snapshot.HitCounts?.Count300 ?? 0, h100 = snapshot.HitCounts?.Count100 ?? 0, h50 = snapshot.HitCounts?.Count50 ?? 0;
                int sliderTail = snapshot.HitCounts?.SliderTailHit ?? 0, smallTick = snapshot.HitCounts?.SmallTickHit ?? 0, largeTick = snapshot.HitCounts?.LargeTickHit ?? 0;
                ...
                if (statsChanged)
                {
                    ...
                    _ppTimeline.Add(new object[] { snapshot.PP ?? 0, snapshot.MaxCombo ?? 0, snapshot.Accuracy ?? 0, h300, h100, h50, misses, snapshot.TimeMs ?? 0, combo, eventType });
    ...
    private CompletedPlay CreateCompletedPlay(LiveSnapshot s, string outcome)
    {
        ...
        {
            ScoreId = 0, CreatedAtUtc = s.ScoreDate?.ToUniversalTime() ?? DateTime.UtcNow, BeatmapHash = s.MD5Hash ?? "", Beatmap = s.Beatmap,
            ...
            Score = s.Score ?? 0, MaxCombo = s.MaxCombo ?? s.Combo ?? 0, Count300 = s.HitCounts?.Count300 ?? 0,
            Count100 = s.HitCounts?.Count100 ?? 0, Count50 = s.HitCounts?.Count50 ?? 0, Misses = s.HitCounts?.Misses ?? 0,
            PP = s.PP ?? 0, TimelineJson = SerializeTimeline(), PpTimelineJson = SerializePpTimeline(),
            AimOffsetsJson = s.AimOffsetsJson, CS = s.CS ?? 0, AR = s.AR ?? 0, OD = s.OD ?? 0, HP = s.HP ?? 0,
            BPM = s.BPM ?? 0, LengthMs = s.TotalTimeMs ?? 0, Circles = s.Circles ?? 0, Sliders = s.Sliders ?? 0,
            Spinners = s.Spinners ?? 0, BackgroundHash = s.BackgroundHash, ReplayHash = s.ReplayHash ?? "", MapPath = s.MapPath ?? ""
        };
    }
